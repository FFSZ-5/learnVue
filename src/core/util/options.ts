...
//判断组件名是否合法，不是组件的name而是componet的key值
...
function normalizeProps(options: Record<string, any>, vm?: Component | null) {
  ...
  if (isArray(props)) {
    ...
    for (const key in props) {
      val = props[key]
      name = camelize(key)
      res[name] = isPlainObject(val) ? val : { type: val } //是对象就传对象，不是对象则作为type
    }
  } else if (__DEV__) {
    warn(
...
export function mergeOptions(
  ...
  //判断是否为函数，这里child只能是配置对象不能是函数，如果是函数证明child是组件
  if (isFunction(child)) {
    // @ts-expect-error
    child = child.options
  }
  ...
  function mergeField(key: any) {
    const strat = strats[key] || defaultStrat //child有值则用child否则用parent
